...
def walk(points, steps, plot_step):
    ...
    position_axes = [xymin, xymax, xymin, xymax]
    ...
    for step in range(steps):
        ...
        if (step + 1) % plot_step == 0;
            plot_title = '%d particles after %d steps' % (particles, step + 1)
            file_name = 'tmp_%03d.pdf' % (step + 1)
            plt.plot(xpositions, ypositions, 'ko', axis = position_axes, title = plot_title, savefig = file_name)

def main(points, steps, plot_step):
    import datetime as dt
    time = dt.datetime.now()
    np.random.seed(time.microsecond)
    x, y = walk(points, steps, plot_step)
